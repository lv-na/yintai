...
//5.获取子节点
// 思路：通过父节点获取子节点，所以要先给一个父节点(即调用相关属性的对象)
...
//删除事件
 ...
 function mouseWheel(obj,funUp,funDown){ //给某一对象添加鼠标滚轮事件
    if(obj.attachEvent){
      obj.attachEvent("onmousewheel",scroll);  //事件处理函数
    } else if(obj.addEventListener){
      obj.addEventListener("DOMMouseScroll",scroll,false);  //火狐
      obj.addEventListener("mousewheel",scroll,false);   //谷歌，IE
    }
    function scroll(e){
      var ev=e||window.event
      var d=ev.wheelDelta||ev.detail;  //如果浏览器是谷歌IE或火狐
      if(d==-120||d==3){     //当值是-120或3时是向下滑动
        if(funDown){
          funDown();
        }
      }else if(d==120||d==-3){  //当值是120或-3时是向下滑动
        if(funUp){
          funUp();
        }
      }
